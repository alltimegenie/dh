...
}
{
  check_for_profit = function () {
    ...
    profits = document.querySelectorAll(".profit-col"); for (profit of profits) {  
    //profit = document.getElementsByClassName("profit-col").length;
    //profit2 = document.querySelectorAll(".profit-col").length;
     // console.warn("warning");  
      console.error(profit.textContent.split(".")[2].split('%')[0]);
      console.error(profit.textContent);
     // console.error(profit2);
    }
  }
...
{check_for_profit_percentage = setInterval(check_for_profit, 5000); go_to_markets_events_id = setInterval(add_bet_finder_events, 100); check_for_autorefresh_id = setInterval(check_for_autorefresh, 100); devigging_events_id = setInterval(add_devigging_events, 100); dim_rows_id = setInterval(dim_rows, 100); delete_old_data = setInterval(delete_old_data, 30000); highlight_parlay_cell_id = setInterval(highlight_parlay_members, 100); load_league(); clear_parlay(); }